#!/usr/bin/env python3
"""
Company Email Backup Script
Backs up Gmail emails to individual Dropbox folders using domain-wide delegation
"""
...
COMPANY_DOMAIN = 'heyhanni.com'
BACKUP_FOLDER_PREFIX = '/Email Backups'

# Gmail API scopes (using working scopes)
SCOPES = [
...
class EmailBackupService:
    def __init__(self):
        self.credentials = self._get_delegated_credentials(ADMIN_EMAIL)
    ...
    def get_company_users(self) -> List[Dict]:
        """Get all users in the company domain"""
        try:
            users = []
            page_token = None
            
            while True:
                result = self.admin_service.users().list(
            ...
            logging.info(f"Found {len(users)} users in domain")
            return users
            
        except HttpError as error:
            logging.error(f"Error getting users: {error}")
            return []
    ...
    def _get_team_member_info(self, email: str) -> Optional[Dict]:
        ...
        try:
            ...
            for header in headers['payload']['headers']:
                if header['name'] == 'Subject':
                    ...
                    try:
                        # Parse date for better filename
    ...
    def run_backup(self, days_back: int = 7):
        """Run backup for all company users"""
        logging.info("Starting company-wide email backup")
        
        users = self.get_company_users()
        if not users:
            logging.error("No users found, aborting backup")
            return
...
def main():
    """Main function"""
    try:
        backup_service = EmailBackupService()
